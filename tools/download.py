...
def download_maps(beatmapset_ids, db_ids, dir):
    ...
    print(
        f"A total of {len(items_not_in_db)} maps are missing, downloading missing maps"
    )

    for set_id in tqdm(items_not_in_db):
        if not os.path.isfile(
            f"{dir}{set_id}.osz"
        ):  # Check if already exist in download folder
            try:
                time.sleep(1)  # Prevent rate limiting
                urlretrieve(
                    f"https://api.nerinyan.moe/d/{set_id}?noVideo=true&NoHitsound=true&NoStoryboard=true",
                    f"{dir}{set_id}.osz",
                )
